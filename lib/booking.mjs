...
export function availability(db, serviceId, date, now = Date.now()) {
  ...
  const occupied = db.prepare("SELECT starts_at, ends_at FROM bookings WHERE date = ? AND status IN ('pending', 'confirmed')").all(date);
  const slots = [];
...
function insertBooking(db, user, input, now) {
  demand(user.verified, 'Verify your email before booking.', 403);
  demand(!db.prepare("SELECT id FROM bookings WHERE user_id = ? AND status IN ('pending','confirmed')").get(user.id), 'You already have an active appointment. Cancel it before booking another.', 409);
  ...
  const settings = settingsFor(db);
  const status = settings.autoApprove && user.approvals >= settings.requiredApprovals && (!slot.outside || !settings.outsideApproval) ? 'confirmed' : 'pending';
  ...
  db.prepare(`INSERT INTO bookings (id,user_id,service_id,service_name,date,time,starts_at,ends_at,duration,price,outside,status,repeat_weeks,notes,created_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, user.id, service.id, service.name, input.date, input.time, slot.startsAt, slot.endsAt, service.duration, slot.price, Number(slot.outside), status, input.repeatWeeks, input.notes.trim(), now);
...
export function changeBooking(db, actor, id, action, now = Date.now()) {
  return transaction(db, () => {
    ...
    demand(actor.role === 'admin' || actor.id === booking.user_id, 'This appointment belongs to another account.', 403);
    ...
    if (action === 'cancel') {
      ...
      demand(actor.role === 'admin', 'Admin access is required.', 403);
      if (action === 'approve') {
        ...
        if (booking.repeat_weeks) {
          ...
          try {
            ...
            const next = insertBooking(db, user, { serviceId: service.id, date, time: booking.time, repeatWeeks: booking.repeat_weeks, notes: booking.notes, expectedPrice: booking.price }, now);
