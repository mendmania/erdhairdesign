import { queueBookingEvent } from './notifications.mjs';
...
function insertBooking(db, user, input, now, manual = null) {
  demand(manual || user?.verified, 'Verify your email before booking.', 403);
  ...
  const settings = settingsFor(db);
  const status = manual || (settings.autoApprove && user.approvals >= settings.requiredApprovals && (!slot.outside || !settings.outsideApproval)) ? 'confirmed' : 'pending';
  ...
  if (manual) {
    db.prepare('UPDATE bookings SET guest_name=?, guest_phone=?, created_by=?, manually_approved=1 WHERE id=?').run(manual.name, manual.phone, manual.actorId, id);
...
export function createAdminBooking(db, actor, input, now = Date.now()) {
  ...
  return transaction(db, () => {
    ...
    return insertBooking(db, user, { ...input, repeatWeeks: 0 }, now, { actorId: actor.id, name, phone });
  });
}
