import { verificationEmail } from './email-templates.mjs';
...
export async function register(db, input) {
  ...
  demand(typeof input.password === 'string' && input.password.length >= 6 && input.password.length <= 200, 'Use a password between 6 and 200 characters.');
...
export async function sendVerification(db, user, { production, apiKey, from, language = 'sq' }) {
  ...
  if (apiKey) {
    ...
    try {
      response = await fetch('https://api.brevo.com/v3/smtp/email', { method: 'POST', headers: { 'api-key': apiKey, 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ sender, to: [{ email: user.email }], ...verificationEmail(code, language) }), signal: AbortSignal.timeout(15000) });
