...
export function setAdmin(db, actor, email, role) {
  ...
  return transaction(db, () => {
    ...
    if (role === 'client') {
      db.prepare('DELETE FROM notification_recipients WHERE user_id = ?').run(user.id);
      db.prepare("UPDATE admin_notifications SET email_status='skipped' WHERE user_id=? AND email_status IN ('pending','failed')").run(user.id);
    }
