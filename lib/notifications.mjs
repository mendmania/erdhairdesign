...
export function queueClientNotification(db, booking, kind, now, previous = null) {
  ...
  const payload = { guest: !booking.user_id, name: client?.name || booking.guest_name, service: booking.service_name, serviceSq: booking.service_name_sq,
    date: booking.date, time: booking.time, status: booking.status, timezone: settingsFor(db).timezone,
    ...(previous ? {previousDate:previous.date, previousTime:previous.time} : {}) };
  db.prepare(`INSERT INTO client_notifications (email_key,booking_id,user_id,kind,schedule_version,payload,language,email_status,next_attempt_at)
    VALUES (?,?,?,?,?,?,?,'pending',?) ON CONFLICT(booking_id,kind,schedule_version) DO NOTHING`)
    .run(randomUUID(), booking.id, booking.user_id, kind, booking.schedule_version, JSON.stringify(payload), booking.email_language, now);
}

export function queueClientConfirmation(db, booking, now) { queueClientNotification(db, booking, 'confirmed', now); }
...
export function bookingEmail(job, appUrl) {
  if (job.queue === 'client_notifications' || ['requested','confirmed','declined','rescheduled'].includes(job.kind)) return clientBookingEmail(job, appUrl);
  ...
  const link = new URL('/admin', appUrl);
  link.searchParams.set('booking', job.booking_id);
  const label = needsApproval ? (sq ? 'Konfirmo rezervimin' : 'Confirm reservation') : (sq ? 'Shiko rezervimin' : 'View reservation');
  const intro = job.kind === 'cancelled' ? (sq ? 'Një rezervim është anuluar. Kalendari i sallonit është përditësuar.' : 'A reservation has been cancelled. Your salon calendar is up to date.')
  ...
  const footer = sq ? 'Ky është një njoftim për ekipin e ERD. Hyni në llogarinë tuaj të administratorit për të menaxhuar rezervimin.' : 'This is an ERD team notification. Sign in to your admin account to manage the reservation.';
...
function clientBookingEmail(job, appUrl) {
  const p = JSON.parse(job.payload), sq = job.language === 'sq';
  const titles = {
    requested: ['Your appointment request was received.', 'E morëm kërkesën tuaj për termin.'],
    confirmed: ['Your reservation is confirmed.', 'Rezervimi juaj është konfirmuar.'],
    declined: ['Your appointment request was declined.', 'Kërkesa juaj për termin u refuzua.'],
    cancelled: ['Your appointment has been cancelled.', 'Termini juaj është anuluar.'],
    rescheduled: ['Your appointment time has changed.', 'Orari i terminit tuaj ka ndryshuar.'],
  };
  const messages = {
    requested: ['The salon will review your request. This is not a confirmation. Please wait for your confirmation before coming in.', 'Salloni do ta shqyrtojë kërkesën tuaj. Ky nuk është konfirmim. Ju lutemi prisni konfirmimin para se të vini.'],
    confirmed: ['Your appointment is secured. We look forward to welcoming you to ERD Hair Design.', 'Termini juaj është rezervuar. Me kënaqësi ju presim në ERD Hair Design.'],
    declined: ['The salon could not accept this request. You can choose another time or contact the salon.', 'Salloni nuk mundi ta pranojë këtë kërkesë. Mund të zgjidhni një orar tjetër ose të kontaktoni sallonin.'],
    cancelled: ['This appointment is no longer reserved. You can choose another time or contact the salon.', 'Ky termin nuk është më i rezervuar. Mund të zgjidhni një orar tjetër ose të kontaktoni sallonin.'],
    rescheduled: ['The salon has moved your appointment to the time below.', 'Salloni e ka zhvendosur terminin tuaj në orarin e mëposhtëm.'],
  };
  const title = titles[job.kind][Number(sq)];
  const awaitingApproval = p.status === 'pending';
  const status = ({pending:['Awaiting approval','Në pritje të miratimit'], confirmed:['Confirmed','Konfirmuar'], declined:['Declined','Refuzuar'], cancelled:['Cancelled','Anuluar']})[p.status][Number(sq)];
  const ended = ['declined','cancelled'].includes(job.kind);
  const path = ended ? '/book' : p.guest ? '/studio' : '/appointments';
  const label = ended ? (sq ? 'Zgjidhni një orar tjetër' : 'Choose another time') : p.guest ? (sq ? 'Studioja jonë' : 'Our studio') : (sq ? 'Terminet e mia' : 'My visits');
  const link = new URL(path, appUrl).href;
  const details = [[sq ? 'Klienti' : 'Client', p.name], [sq ? 'Shërbimi' : 'Service', sq && p.serviceSq ? p.serviceSq : p.service]];
  if (p.previousDate) details.push([sq ? 'Orari i mëparshëm' : 'Previous time', `${p.previousDate} · ${p.previousTime}`]);
  details.push([sq ? 'Data' : 'Date', p.date], [sq ? 'Ora' : 'Time', p.time], [sq ? 'Statusi' : 'Status', status]);
  const intro = `${p.name}, ${messages[job.kind][Number(sq)]}${job.kind === 'rescheduled' && awaitingApproval ? (sq ? ' Kërkesa ende pret miratimin. Ju lutemi prisni konfirmimin para se të vini.' : ' Your request still needs approval. Please wait for confirmation before coming in.') : ''}`;
  const footer = ended ? (sq ? 'Nuk keni nevojë të vini në orarin e treguar më sipër.' : 'You do not need to attend at the time shown above.')
    : p.guest ? (sq ? 'Për të ndryshuar ose anuluar rezervimin, kontaktoni sallonin.' : 'To change or cancel your reservation, please contact the salon.')
    : (sq ? 'Për të parë ose anuluar rezervimin, hyni në llogarinë tuaj dhe hapni Terminet e mia.' : 'To view or cancel your reservation, sign in to your account and open My visits.');
  return {subject:`${title} · ${p.date} ${p.time} · ERD Hair Design`,
    textContent:[title,intro,...details.map(([key,value]) => `${key}: ${value}`),`${label}: ${link}`,footer].join('\n'),
    htmlContent:emailLayout({language:job.language,title,intro,details,action:{label,url:link},footer})};
}
    ...
    for (let i = 0; i < 20 && !stopped; i++) {
      const job = transaction(db, () => {
        ...
        const row = db.prepare(`SELECT n.id,n.email_key,n.user_id,n.language,n.attempts,n.next_attempt_at,
          e.kind,e.booking_id,e.payload,u.email,u.verified,u.role,b.status AS booking_status,b.date AS booking_date,b.time AS booking_time,
          b.schedule_version, b.schedule_version AS current_version,
          ...
          n.kind,n.booking_id,n.payload,COALESCE(u.email,b.guest_email) AS email,u.verified,u.role,b.status AS booking_status,b.date AS booking_date,b.time AS booking_time,
          n.schedule_version, b.schedule_version AS current_version,
        ...
        const currentTime = payload.date === row.booking_date && payload.time === row.booking_time;
        const eligible = client ? row.booking_status === payload.status && row.schedule_version === row.current_version && currentTime
          : row.verified && ['admin','super_admin'].includes(row.role) && (row.kind !== 'created' || (['pending','confirmed'].includes(row.booking_status) && currentTime));
        if (!eligible) {
