...
function adminPage() {
  if (!isStaff()) return `<section class="content-page">${heading('THE SALON WORKSPACE', 'A little behind <em>the scenes.</em>', 'Sign in with your salon administrator account to manage appointments.')}<button class="button button-primary" data-action="login">Admin sign in ${icon('arrow')}</button></section>`;
...
async function action(button) {
  ...
  if (a === 'admin-entry') { if (isStaff()) location.hash = 'admin'; else { state.authIntent = 'admin'; authModal(); } }
  if (a === 'book') {
