...
function updateNav() {
  ...
  $('#account-nav').innerHTML = state.user ? `${isStaff() ? '<a class="account-link workspace-link" href="#admin">Workspace</a>' : ''}<a class="account-link" href="#appointments">${icon('calendar')}<span>My visits</span></a><button class="avatar" data-action="account" aria-label="Your account">${esc(state.user.name.slice(0, 1))}</button>` : `<button class="button button-outline" data-action="login">Sign in <span aria-hidden="true">↗</span></button>`;
}
function render() {
...
function serviceStep() {
  ...
  return `<div class="section-title"><div><p class="eyebrow">01 / THE GOOD PART</p><h2>What brings you in?</h2><p>Choose a little something for your hair.</p></div><span class="small-label">Made for you ${icon('sparkles')}</span></div>
    ...
    <div class="service-grid">${filtered.map(s => serviceCard(s)).join('') || '<p class="empty-slots">No services available here yet. Please check back soon.</p>'}</div>
...
function timeStep() {
  ...
  return `<div class="section-title"><div><p class="eyebrow">02 / MAKE SOME SPACE</p><h2>When works for you?</h2><p>Find a moment that fits into your day.</p></div></div>
    ...
    <p class="slot-heading">${icon('sun')} During salon hours <span>${money(service().price)}</span></p><div class="time-grid">${regular.length ? regular.map(slotButton).join('') : `<p class="empty-slots">${state.closed ? 'The salon is taking time off on this date. Please choose another day.' : 'No regular-hour times on this day. Try another date.'}</p>`}</div>
...
function adminPage() {
  if (!isStaff()) return `<section class="content-page">${heading('THE SALON WORKSPACE', 'Salon <em>sign in.</em>', state.user ? `Signed in as ${esc(state.user.email)}. Administrator access is required.` : 'Sign in with your salon administrator account.')}<button class="button button-primary" data-action="login">${state.user ? 'Use another account' : 'Admin sign in'} ${icon('arrow')}</button></section>`;
  ...
  return `<section class="content-page admin-page page-enter"><header class="workspace-heading"><div><p class="eyebrow">SALON OPERATIONS</p><h1>Salon workspace</h1><p>${dateLabel(today, {year:'numeric'})} <span aria-hidden="true">·</span> ${vacation ? `Time off: ${esc(vacation.label)}` : shift.open ? `Open ${shift.start}–${shift.end}` : 'No regular shift today'}</p></div><div class="workspace-heading-actions"><span class="owner-badge">${isOwner() ? 'Super admin' : 'Administrator'}</span><button class="button button-outline button-small" data-action="refresh-admin">${icon('repeat')} Refresh</button></div></header>
    <div class="operations-stats">${[['today',"Today's visits",'Your daily schedule'],['pending','Awaiting approval','Review client requests'],['complete','Ready to complete','Finish visits & renew repeats'],['upcoming','Upcoming','All future active visits']].map(([view,label,hint]) => `<button class="operation-stat ${state.adminTab === 'appointments' && state.adminFilter === view ? 'selected' : ''}" data-action="admin-view" data-view="${view}"><span>${label}</span><strong>${counts[view]}</strong><small>${hint} ↗</small></button>`).join('')}</div>
    <nav class="admin-tabs" aria-label="Salon workspace sections">${['appointments', 'hours', 'vacations', 'prices', 'rules', ...(isOwner() ? ['team'] : [])].map(t => `<button data-action="admin-tab" data-tab="${t}" aria-current="${state.adminTab === t ? 'page' : 'false'}" class="${state.adminTab === t ? 'selected' : ''}">${({ appointments: 'Appointments', hours: 'Working hours', vacations: 'Time off', team: 'Administrators', prices: 'Services & prices', rules: 'Booking rules' })[t]}</button>`).join('')}</nav>
...
function adminResults() {
  ...
  let date = '';
  return `<p class="result-count" role="status">${bookings.length} ${bookings.length === 1 ? 'appointment' : 'appointments'} · ${adminViews[state.adminFilter]}</p>${bookings.length ? `<div class="appointment-list">${bookings.map(b => { const heading = b.date !== date ? `<h3 class="agenda-day">${b.date === state.today ? 'Today · ' : ''}${dateLabel(b.date, {year:'numeric'})}</h3>` : ''; date = b.date; return heading + adminAppointmentCard(b); }).join('')}</div>` : `<div class="empty-state operations-empty">${icon('calendar')}<h3>${state.adminQuery || state.adminDate ? 'No matching appointments' : state.adminFilter === 'pending' ? 'All requests reviewed' : state.adminFilter === 'complete' ? 'Nothing waiting to be completed' : state.adminFilter === 'today' ? 'Your day is clear' : 'No appointments here'}</h3><p>${state.adminQuery || state.adminDate ? 'Try another search or clear the filters.' : 'Use the views above to check other appointments.'}</p><button class="button button-outline button-small" data-action="admin-view" data-view="all">View all appointments</button></div>`}`;
}
function adminAppointmentCard(b) {
  ...
  return `<article class="operation-appointment"><div class="agenda-time"><strong>${b.time}</strong><span>${duration(b.duration)}</span></div><div class="agenda-detail"><div class="appointment-title"><h3>${esc(b.name)}</h3><span class="badge badge-${b.status}">${expired ? 'Expired request' : ready ? 'Ready to complete' : ({pending:'Needs approval',confirmed:'Confirmed',completed:'Completed',cancelled:'Cancelled',declined:'Declined'})[b.status]}</span></div><p class="agenda-service">${esc(b.service_name)} <strong>${money(b.price)}</strong>${b.outside ? ' · Outside hours' : ''}</p><div class="agenda-contact">${phone ? `<a href="tel:${esc(phone)}">${esc(b.phone)}</a>` : ''}<a href="mailto:${esc(b.email)}">${esc(b.email)}</a></div>${b.notes ? `<p class="agenda-note"><strong>Client note:</strong> ${esc(b.notes)}</p>` : ''}${b.repeat_weeks ? `<p class="repeat-caption">${icon('repeat')} Every ${b.repeat_weeks} week${b.repeat_weeks === 1 ? '' : 's'}</p>` : ''}${b.recurrence_note ? `<p class="recurrence-note">${esc(b.recurrence_note)}</p>` : ''}${expired ? '<p class="agenda-warning">This time has passed. Decline the request so the client can book again.</p>' : ''}${b.status === 'pending' && !expired ? `<p class="fine-print">${b.approvals} manual approvals so far</p>` : ''}</div><div class="agenda-actions">${b.status === 'pending' ? `${!expired ? `<button class="button button-primary button-small" data-action="booking-action" data-id="${b.id}" data-value="approve">Approve ${icon('check')}</button>` : ''}<button class="text-button danger" data-action="admin-decline-open" data-id="${b.id}">Decline request</button>` : ready ? `<button class="button button-primary button-small" data-action="${b.repeat_weeks ? 'admin-complete-open' : 'booking-action'}" data-id="${b.id}" data-value="complete">Complete visit ${icon('check')}</button>` : b.status === 'confirmed' ? '<span class="fine-print">Complete after the visit ends</span>' : ''}${active ? `<button class="text-button subtle" data-action="admin-cancel-open" data-id="${b.id}">Cancel visit</button>` : ''}</div></article>`;
}
function hoursForm() {
  const s = state.admin.settings;
  return `<form id="hours-form" class="admin-form"><h2>Your working week</h2><p>Clients can book a full service within these shifts. Times are in ${esc(s.timezone)}. Existing appointments keep their agreed time and price.</p><div class="shift-tools"><button class="button button-outline button-small" type="button" data-action="copy-weekdays">Copy Monday to Tue–Fri</button><span>Then save to apply your changes.</span></div><div class="shift-grid">${s.shifts.map(day => `<div class="shift-row"><label class="switch-label"><input type="checkbox" name="open-${day.day}" ${day.open ? 'checked' : ''}/><span>${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][day.day]}</span></label><input type="time" name="start-${day.day}" value="${day.start}" aria-label="${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][day.day]} start" required/><span>to</span><input type="time" name="end-${day.day}" value="${day.end}" aria-label="${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][day.day]} end" required/></div>`).join('')}</div><h3>A little outside the usual hours</h3><label class="switch-label"><input type="checkbox" name="allowOutside" ${s.allowOutside ? 'checked' : ''}/>Allow requests outside working shifts, including closed days</label><div class="field-grid"><label>Earliest request time<input type="time" name="outsideStart" value="${s.outsideStart}" required/></label><label>Latest finish time<input type="time" name="outsideEnd" value="${s.outsideEnd}" required/></label></div><p class="form-error" id="form-error" role="alert"></p><div class="admin-save-bar"><span>Changes apply when you save.</span><button class="button button-primary" type="submit">Save working hours ${icon('check')}</button></div></form>`;
}
function pricesForm() {
  return `<div class="admin-form"><div class="admin-section-heading"><div><h2>Your service menu</h2><p>Add just what you offer. Prices are in ${esc(state.settings.currency)}; changes apply to new bookings.</p></div><button class="button button-primary" data-action="service-edit">Add service +</button></div><div class="admin-items">${state.admin.services.map(s => `<article class="admin-item"><div><h3>${esc(s.name)}</h3><p>${esc(s.category)} · ${duration(s.duration)}</p><p>${money(s.price)} in hours · ${money(s.outside_price)} outside hours</p></div><div class="admin-item-actions"><button class="button button-outline" data-action="service-edit" data-id="${s.id}" aria-label="Edit ${esc(s.name)}">Edit</button><button class="text-button" data-action="service-remove-open" data-id="${s.id}" aria-label="Remove ${esc(s.name)}">Remove</button></div></article>`).join('') || '<p class="empty-slots">No services yet. Add your first service to open bookings.</p>'}</div></div>`;
}
...
async function action(button) {
  ...
  if (a === 'copy-weekdays') { const form = $('#hours-form'); for (let day = 2; day <= 5; day++) { form.elements[`open-${day}`].checked = form.elements['open-1'].checked; form.elements[`start-${day}`].value = form.elements['start-1'].value; form.elements[`end-${day}`].value = form.elements['end-1'].value; } toast('Monday copied to Tuesday–Friday. Save working hours to apply.'); }
  if (['admin-decline-open','admin-cancel-open','admin-complete-open'].includes(a)) {
    ...
    openModal(`<h2 id="modal-title">${value === 'complete' ? 'Complete this repeat visit?' : value === 'decline' ? 'Decline this request?' : 'Cancel this visit?'}</h2><p class="modal-description"><strong>${esc(b.name)}</strong><br>${esc(b.service_name)} · ${dateLabel(b.date)} at ${b.time}</p><p class="modal-description">${value === 'complete' ? 'This finishes the visit and requests the next repeat, subject to availability and booking rules.' : 'The time will be released and any repeat schedule will stop. The client can see the update in My visits.'}</p><p class="form-error" id="modal-error" role="alert"></p><button class="button button-primary full-width" data-action="booking-action" data-value="${value}" data-id="${b.id}">${value === 'complete' ? 'Complete & renew repeat' : value === 'decline' ? 'Decline request' : 'Cancel visit'}</button><button class="text-button full-width cancel-keep" data-action="modal-close">Go back</button>`);
  }
...
document.addEventListener('input', event => { if (event.target.name === 'notes') state.notes = event.target.value; if (event.target.id === 'admin-search') { state.adminQuery = event.target.value; $('#admin-results').innerHTML = adminResults(); } });
document.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (form.id === 'service-form') { await api(`/admin/services${data.serviceId ? `/${data.serviceId}` : ''}`, data.serviceId ? 'PUT' : 'POST', {...data, duration: Number(data.duration), price: Math.round(Number(data.price) * 100), outside_price: Math.round(Number(data.outside_price) * 100)}); closeModal(); await route(); toast('Service saved.'); }
    if (form.id === 'vacation-form') { await api('/admin/vacations', 'POST', data); await route(); toast('Time off added. These dates are closed for bookings.'); }
    if (form.id === 'team-form') { await api('/admin/team', 'PUT', {email: data.email, role: 'admin'}); await route(); toast('Administrator access granted. They can sign in again now.'); }
