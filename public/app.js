...
const bookingAsGuest = () => !state.user?.verified && state.bookingMode !== 'account';
...
function bookingPage() {
  return h`<section class="booking-page page-enter">${heading(t('HAIR. CARE. YOU.'), h('Good hair.<br><em>Zero fuss.</em>'), h('Choose a service. Pick your time. We’ll take care of the rest.'))}
    ...
    <div class="booking-promises"><span>${icon('shield')} Easy booking. Personal care.</span><span>${icon('clock')} Pay at the salon</span></div>
  </section>`;
}
function serviceStep() {
  ...
  return h`<div class="section-title"><div><p class="eyebrow">01 / YOUR SERVICE</p><h2>Choose your service.</h2><p>Tap a service to see available times.</p></div></div>
    ...
    <p class="pricing-note">${icon('shield')} Book as a guest or sign in to your account.</p>`;
}
...
function timeStep() {
  ...
  return h`<div class="section-title"><div><p class="eyebrow">02 / YOUR TIME</p><h2>Pick your time.</h2><p>${esc(serviceName(service()))} · ${duration(service().duration)}</p></div></div>
    ...
    `}
    ${state.user?.verified ? h`<details class="booking-options" ${state.repeatWeeks ? 'open' : ''}><summary>Repeat this visit <span class="optional">Optional</span>${icon('repeat')}</summary><div class="repeat-option"><span class="repeat-icon">${icon('repeat')}</span><div><strong>Make it your regular thing</strong><p>One visit at a time. Your next is booked after you come in.</p></div><select id="repeat-weeks" aria-label="Repeat appointment">${[0, 1, 2, 4, 6, 8].map(w => h`<option value="${w}" ${w === state.repeatWeeks ? 'selected' : ''}>${w ? h`Every ${w} week${w > 1 ? 's' : ''}` : h('Just this once')}</option>`).join('')}</select></div></details>` : h('<p class="fine-print">Sign in to book repeating visits.</p>')}<p class="timezone-note">All appointment times are in ${esc(state.settings.timezone)}.</p>`;
}
...
function detailsStep() {
  return h`<div class="section-title"><div><p class="eyebrow">03 / YOUR DETAILS</p><h2>Your details.</h2><p>A few details, so we’re ready to welcome you.</p></div></div>
    ${!state.user?.verified ? h`<div class="booking-modes" role="group" aria-label="Booking options"><button type="button" data-action="booking-mode" data-mode="guest" aria-pressed="${bookingAsGuest()}">Book as a guest</button><button type="button" data-action="booking-mode" data-mode="account" aria-pressed="${!bookingAsGuest()}">Use an account</button></div>` : ''}
    ${bookingAsGuest() ? guestDetailsForm() : !state.user ? h`<div class="account-gate"><span class="gate-icon">${icon('user')}</span><h3>Your good hair days start here.</h3><p>Create an account and verify your email to book.<br>Already part of the studio? Welcome back.</p><button class="button button-primary" data-action="register">Create an account ${icon('arrow')}</button><p class="signin-caption">Already have an account? <button class="text-button" data-action="login">Sign in</button></p><span class="gate-assurance">${icon('shield')} Your details stay private and secure.</span></div>` : !state.user.verified ? h`<div class="account-gate"><span class="gate-icon">${icon('mail')}</span><h3>One small step: verify your email.</h3><p>Confirm ${esc(state.user.email)} to book your visit.</p><button class="button button-primary" data-action="verify-open">Verify email ${icon('arrow')}</button></div>` : h`<form id="details-form" class="details-form"><div class="verified-banner">${icon('shield')} Email verified <span>${esc(state.user.email)}</span></div><div class="field-grid"><label>Full name<input name="name" autocomplete="name" value="${esc(state.detailsDraft?.name ?? state.user.name)}" required minlength="2" maxlength="100" placeholder="Your full name"/></label><label>Phone number<input name="phone" type="tel" autocomplete="tel" value="${esc(state.detailsDraft?.phone ?? state.user.phone)}" required placeholder="+381 …"/></label></div><label>Anything we should know? <span class="optional">Optional</span><textarea name="notes" maxlength="1000" rows="2" placeholder="Your hair goals, preferences, or anything that helps us make your visit feel like you.">${esc(state.notes)}</textarea></label><p class="form-error" id="form-error" role="alert"></p><div class="form-footer"><button type="button" class="text-button" data-action="step" data-step="1">← Back</button><button class="button button-primary" type="submit">Review your visit ${icon('arrow')}</button></div></form>`}`;
}
function guestDetailsForm() {
  const details = state.guestDetails || {};
  return h`<form id="guest-details-form" class="details-form"><p class="fine-print">No account needed. We’ll email you when the salon confirms your request.</p>
    <label>Full name<input name="name" autocomplete="name" value="${esc(details.name)}" required minlength="2" maxlength="100" placeholder="Your full name"/></label>
    <label>Email address<input name="email" type="email" autocomplete="email" value="${esc(details.email)}" required maxlength="254" placeholder="you@example.com"/></label>
    <label>Phone number (optional)<input name="phone" type="tel" autocomplete="tel" value="${esc(details.phone)}" maxlength="30" placeholder="Your phone number"/></label>
    <label>Anything we should know? <span class="optional">Optional</span><textarea name="notes" maxlength="1000" rows="2">${esc(state.notes)}</textarea></label>
    <p class="form-error" id="form-error" role="alert"></p><div class="form-footer"><button type="button" class="text-button" data-action="step" data-step="1">← Back</button><button class="button button-primary" type="submit">Review your visit ${icon('arrow')}</button></div></form>`;
}
function confirmStep() {
  const guest = bookingAsGuest(), client = guest ? state.guestDetails : state.user;
  const pending = guest || !state.settings.autoApprove || state.user.approvals < state.settings.requiredApprovals || (state.slot.outside && state.settings.outsideApproval);
  return h`<div class="section-title"><div><p class="eyebrow">04 / REVIEW & BOOK</p><h2>Ready when you are.</h2><p>Check your details, then send your booking.</p></div></div>
    ...
    <dl class="review-list"><div><dt>Your moment</dt><dd>${dateLabel(state.date)} at ${state.slot.time}</dd></div><div><dt>Booked for</dt><dd>${esc(client.name)}</dd></div><div><dt>Email</dt><dd>${esc(client.email)}</dd></div>${client.phone ? h`<div><dt>Phone</dt><dd>${esc(client.phone)}</dd></div>` : ''}<div><dt>Your routine</dt><dd>${state.repeatWeeks ? h`Every ${state.repeatWeeks} week${state.repeatWeeks > 1 ? 's' : ''}` : h('Just this once')}</dd></div>${state.notes ? h`<div><dt>A little note</dt><dd>${esc(state.notes)}</dd></div>` : ''}</dl>
    <div class="info-box">${icon(pending ? 'clock' : 'check')}<div><strong>${pending ? h('A quick check from our team.') : h('You’re ready for instant confirmation.')}</strong><p>${guest ? h('We’ll review your request and email your confirmation. Please wait for it before coming in. To change or cancel your request, contact the salon.') : pending ? h('We’ll review your request. Check My visits for your confirmation before coming in.') : h('Your appointment will be confirmed as soon as you book.')} Payment is at the salon.</p></div></div>
    ...
    <p class="form-error" id="form-error" role="alert"></p><div class="form-footer"><button class="text-button" data-action="step" data-step="2">← Back</button><button class="button button-primary" data-action="book">${pending ? h('Request appointment') : h('Confirm appointment')} ${icon('arrow')}</button></div>`;
}
...
async function adminReserveModal() {
  ...
  openModal(h`<p class="eyebrow">PHONE & MESSAGE BOOKINGS</p><h2 id="modal-title">New reservation</h2><p class="modal-description">The appointment is confirmed immediately.</p>
    <form id="admin-reserve-form">
      ...
      <fieldset id="reserve-guest" class="reserve-guest"><label>Full name<input name="name" minlength="2" maxlength="100" autocomplete="off" required placeholder="Client name"/></label><details class="optional-contact"><summary>Add contact details (optional)</summary><p class="fine-print">Add an email to send a confirmation, or a phone number for easy contact.</p><label>Email address (optional)<input name="email" type="email" maxlength="254" autocomplete="off" placeholder="you@example.com"/></label><label>Phone number (optional)<input name="phone" type="tel" maxlength="30" autocomplete="off" placeholder="+383 …"/></label></details></fieldset></fieldset>
...
async function adminRescheduleModal(id) {
  const booking = state.admin.bookings.find(b => b.id === id);
  if (!booking) return;
  openModal(h`<p class="eyebrow">APPOINTMENT DETAILS</p><h2 id="modal-title">Change appointment time</h2>
    <p class="modal-description">The original appointment stays reserved until you save the new time.</p>
    <div class="info-box"><div><strong>${esc(booking.name)} · ${esc(bookingName(booking))}</strong><p>Current appointment</p><p>${dateLabel(booking.date)} · ${booking.time} · ${duration(booking.duration)} · ${money(booking.price)}</p></div></div>
  ...
  $('#modal').classList.add('reservation-dialog');
  $('#admin-reschedule-form').booking = booking;
  await loadRescheduleSlots();
}
function updateRescheduleSummary(form = $('#admin-reschedule-form')) {
  if (!form?.isConnected) return;
  const slot = form.slots?.find(s => s.time === form.elements.time.value);
  const changed = slot && (form.elements.date.value !== form.booking.date || slot.time !== form.booking.time);
  $('#reschedule-summary').innerHTML = changed ? h`<div><strong>New appointment</strong><span>${dateLabel(form.elements.date.value)} · ${slot.time}</span></div><strong>${money(form.booking.price)}</strong>` : h('<span>Choose a different date or time.</span>');
  $('button[type="submit"]',form).disabled = !changed || form.stale || Boolean(form.dataset.submitting);
}
async function loadRescheduleSlots(form = $('#admin-reschedule-form')) {
  if (!form?.isConnected) return;
  const request = (form.slotRequest || 0) + 1; form.slotRequest = request;
  const time = form.elements.time, selected = time.value || form.booking.time;
  const status = $('#reschedule-status'), retry = $('[data-action="reschedule-retry"]',form);
  form.slots = []; form.stale = false; time.disabled = true; retry.hidden = true;
  time.innerHTML = h('<option value="">Choose a time</option>'); updateRescheduleSummary(form);
  ...
  status.textContent = t('Loading available times…');
  try {
    const result = await api(`/admin/bookings/${encodeURIComponent(form.booking.id)}/availability?date=${encodeURIComponent(form.elements.date.value)}`);
    if (!form.isConnected || request !== form.slotRequest) return;
    if (result.scheduleVersion !== form.booking.schedule_version || result.status !== form.booking.status) {
      form.stale = true;
      throw new Error(t('This appointment changed. Close this dialog and refresh the schedule before trying again.'));
    }
    ...
    if (!form.isConnected || request !== form.slotRequest) return;
    status.textContent = error.message; retry.hidden = form.stale;
  }
}
...
function notificationsPage() {
  ...
  return h`<section class="notifications-page"><div class="notification-feed">${notificationFeed()}</div>
    ${isOwner() && prefs ? h`<details class="email-settings" ${adminDrafts.has("notification-settings-form") ? 'open' : ''}><summary>${icon("mail")} Email notification settings <span class="owner-badge">Super admin</span></summary><form id="notification-settings-form" class="admin-form"><h2>Email notifications</h2>
      <p>Every verified administrator receives an email for each new reservation, including reservations they create.</p>
      <p>Clients and guests with an email receive a confirmation. Cancellation alerts go to the other administrators.</p>
      ${!prefs.emailConfigured ? h('<p class="info-box">Email delivery is not configured. Reservation emails stay queued until delivery is configured.</p>') : ''}
      <fieldset class="notification-recipients"><legend>Email recipients</legend>${prefs.recipients.map(u => h`<div class="notification-recipient"><span><strong>${esc(u.name)}</strong><small>${esc(u.email)}</small></span></div>`).join('')}</fieldset>
      <p class="fine-print">Add administrators in the Administrators section. This language applies to future team emails; client emails use the reservation language.</p>
      ...
      <p class="form-error" id="form-error" role="alert"></p>${adminSaveBar("Save email settings")}
    </form></details>` : ''}
  </section>`;
}
...
async function action(button) {
  ...
  if (a === 'booking-mode') { state.bookingMode = button.dataset.mode; if (bookingAsGuest()) state.repeatWeeks = 0; render(); }
  ...
  if (a === 'book') {
    const guest = bookingAsGuest();
    const { booking } = await api(guest ? '/bookings/guest' : '/bookings', 'POST', { ...(guest ? state.guestDetails : {}), serviceId: state.serviceId, date: state.date, time: state.slot.time, repeatWeeks: state.repeatWeeks, notes: state.notes, expectedPrice: state.slot.price });
    state.step = 0; state.serviceId = null; state.slot = null; state.repeatWeeks = 0; state.notes = '';
    state.guestDetails = null; state.bookingMode = null;
    await navigate(guest ? '/book' : '/appointments');
    openModal(h`<span class="success-circle">${icon('check')}</span><p class="eyebrow">A LITTLE TIME FOR YOU</p><h2 id="modal-title">${booking.status === 'pending' ? h('Your request is in.') : h('It’s a date.')}</h2><p class="modal-description">${guest ? h('Your request has been sent. We’ll email you when the salon confirms your appointment. To make changes, contact the salon.') : booking.status === 'pending' ? h('Our team will review your appointment. You can follow its status in My visits.') : h('Your appointment is confirmed. We look forward to seeing you.')}</p><div class="success-detail"><strong>${esc(bookingName(booking))}</strong><span>${dateLabel(booking.date)} · ${booking.time}</span><span>${money(booking.price)} · Pay at the salon</span></div><button class="button button-primary full-width" data-action="modal-close">Lovely, thank you ${icon('check')}</button>`);
  }
  ...
  if (a === 'admin-reschedule') await adminRescheduleModal(button.dataset.id);
  if (a === 'reschedule-retry') await loadRescheduleSlots();
...
document.addEventListener('change', async event => {
  ...
  try {
    ...
    if (event.target.id === 'reschedule-date') await loadRescheduleSlots();
    if (event.target.id === 'reschedule-time') updateRescheduleSummary();
...
document.addEventListener('input', event => {
  ...
  if (event.target.form?.id === 'guest-details-form' && ['name','email','phone'].includes(event.target.name)) state.guestDetails = {...state.guestDetails, [event.target.name]:event.target.value};
  if (event.target.name === 'notes' && ['details-form','guest-details-form'].includes(event.target.form?.id)) state.notes = event.target.value;
...
document.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (form.id === 'guest-details-form') { state.guestDetails = {name:data.name.trim(),email:data.email.trim(),phone:data.phone.trim()}; state.notes = data.notes; state.repeatWeeks = 0; state.step = 3; render(); focusBookingStep(); }
    ...
    if (form.id === 'admin-reschedule-form') {
      if (!form.slots?.some(s => s.time === data.time) || form.stale) throw new Error(t('Choose an available time.'));
      try {
        await api(`/admin/bookings/${encodeURIComponent(form.booking.id)}/reschedule`, 'POST', {...data, expectedVersion:form.booking.schedule_version, expectedStatus:form.booking.status});
      } catch (error) { await loadRescheduleSlots(form); throw error; }
      closeModal(); state.adminFilter = 'all'; state.adminQuery = ''; state.adminDate = data.date; state.adminBookingId = null;
      await navigate('/admin'); toast(h('Appointment time updated.'));
    }
    if (form.id === 'notification-settings-form') {
      state.admin.notificationSettings = await api('/admin/notification-settings', 'PUT', {language:data.language});
  ...
  finally { delete form.dataset.submitting; lockedControls.forEach(el => { el.disabled = false; }); if (draftForms.has(form.id)) syncAdminForm(form); if (submit) { submit.disabled = form.id === 'admin-reserve-form' && !form.elements.time.value; submit.removeAttribute('aria-busy'); } if (form.id === 'admin-reserve-form' && form.isConnected) { form.elements.time.disabled = !form.slots?.length; updateReserveSummary(); } if (form.id === 'admin-reschedule-form' && form.isConnected) { form.elements.time.disabled = !form.slots?.length; updateRescheduleSummary(form); } }
});
