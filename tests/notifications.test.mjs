...
test('only the owner can select verified admin recipients, with atomic validation and no default recipients',t=>{
  ...
  select(db,owner,['owner'],'sq');
  for (const recipientIds of [['admin','client'],['unverified'],['missing'],['admin','admin'],[1],null]) {
    assert.throws(()=>saveNotificationPreferences(db,owner,{recipientIds,language:'en'}));
    assert.equal(notificationPreferences(db,owner).language,'sq');
    assert.deepEqual(notificationPreferences(db,owner).recipients.filter(u=>u.selected).map(u=>u.id),['owner']);
  }
});

test('booking events and unread inboxes persist atomically and exclude unverified staff and the actor',t=>{
  ...
  assert.equal(notificationInbox(db,'admin').unreadCount,1);
...
test('marking notifications read is per administrator and never clears later arrivals',t=>{
  ...
  assert.ok(jobs(db).every(n=>n.email_status==='off'));
});

test('recipient removal and demotion stop queued emails; re-enabling does not resend old events',async t=>{
  const {db,owner}=fixture(t); select(db,owner);
  createBooking(db,'client',input(),NOW);
  select(db,owner,['admin']);
  assert.equal(jobs(db).find(n=>n.user_id==='owner').email_status,'skipped');
  ...
  await worker.kick(); await worker.stop(); assert.equal(sent,0);
});
...
test('email failures retry with backoff, stop after eight attempts, and can be retried by the owner',async t=>{
  const {db,owner}=fixture(t); select(db,owner,['owner']); createBooking(db,'client',input(),NOW);
...
test('email is queued across a database restart and missing configuration never sends or consumes retries',async t=>{
  ...
  select(db,owner,['owner']);createBooking(db,'client',input(),NOW);
...
test('provider failures and duplicate acknowledgements are handled without leaking provider output',async t=>{
  const {db,owner}=fixture(t);select(db,owner,['owner']);createBooking(db,'client',input(),NOW);const job=jobs(db)[0];
