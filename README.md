...
- **Rescheduling:** choose **Change time** on a future pending or confirmed appointment. Review the current and proposed times, then choose **Save new time**. The original slot stays reserved until the change succeeds; availability excludes only that appointment and still respects other visits, working hours, and time off. The agreed service, duration, price (including when moving between regular and outside hours), notes, approval status, and repeat interval are preserved. Future repeats follow the new weekday/time. Clients with an email receive the updated details; tell name-only/phone-only guests directly. Changes record the administrator and old/new times. An outdated dialog cannot overwrite another administrator’s move or approval. Started, cancelled, declined, and completed visits cannot be moved.
...
Booking notification jobs are saved in SQLite in the same transaction as the reservation. A background worker uses the existing `BREVO_API_KEY`, `EMAIL_FROM`, and `APP_URL`; no separate email service is needed. Each recipient receives an individual HTML email with an ivory, green and gold design and a plain-text alternative. Reservation emails include the client name, service, salon date/time, status, and a link to the admin workspace, the client’s My visits page, or the public studio page for guests. Account pages require sign-in. Verification emails share the same design. Notes and phone numbers are not included. Failures retry with increasing delays, up to eight attempts; the super admin can inspect queued/failed counts and retry failed emails in **Notifications**. Jobs survive restarts. Delivery rechecks administrator access and skips stale new-reservation emails for cancelled/completed visits and client confirmations for visits that are no longer confirmed. An email already being sent cannot be recalled. When email is unconfigured locally, jobs remain queued and no messages are sent. Email language settings apply to future events. Past inbox items and pre-existing confirmed appointments are not emailed retroactively. Queue counts and retries include both team and client emails. Client emails cover request receipts, confirmations, declines, cancellations, and time changes. Decline/cancellation messages link to rebooking; a moved pending request remains explicitly unconfirmed. Outbox entries are unique per appointment, event kind, and schedule version. Delivery suppresses obsolete client statuses and appointment times, including older rescheduling messages. Existing queued confirmations keep their delivery state and provider idempotency keys during the upgrade.
