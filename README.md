...
- **Navigation and forms:** desktop navigation groups daily work and salon settings; phones use a section picker and appointment-view selector with counts. Date selection shows visits on that date; searching from Today searches all visits. Phone search ignores spacing and punctuation. Unsaved hours, rules, email recipients, time-off entries, and administrator email entries survive section changes and the panel’s Refresh button, with a **Reset changes** control. Drafts stay only in memory and are cleared by a browser reload or signing out. Save buttons remain reachable while scrolling.
- **Manual reservations:** choose **New reservation** from any workspace section. Search for a registered client by name, email, or phone, or enter a guest’s name and phone number. Choose a service, date, available time, and optional notes; the total stays visible above **Confirm reservation**. The reservation is immediately confirmed, blocks the shared calendar, and can be cancelled or completed like any other visit. No account or email verification is needed for a guest; existing-account bookings appear in **My visits** and count as a manual approval. These are one-off visits, respect hours/time off/prices, and retain the one-active-visit limit for registered accounts.
